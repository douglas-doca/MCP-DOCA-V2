...
class SchedulerService {
    ...
    getSchedulerUrl(clientId) {
        const config = clientService.getClientConfig(clientId);
        if (!config)
            return null;
        return config.scheduler_url || null;
    }
    async consultarHorarios(clientId, data) {
    ...
    detectSchedulingIntent(message) {
        ...
        if (/hoje/i.test(msg))
            data = 'hoje';
        else if (/amanh[ãa]/i.test(msg))
            data = 'amanha';
        else if (/segunda/i.test(msg))
            data = 'segunda';
        else if (/ter[çc]a/i.test(msg))
            data = 'terca';
        else if (/quarta/i.test(msg))
            data = 'quarta';
        else if (/quinta/i.test(msg))
            data = 'quinta';
        else if (/sexta/i.test(msg))
            data = 'sexta';
        else if (/s[aá]bado/i.test(msg))
            data = 'sabado';
        // Extrair horário
        let horario;
    ...
    formatHorariosParaPrompt(horarios, data) {
