...
// NORMALIZAÇÃO Z-API
// ============================================================
function normalizeZapi(payload) {
...
// HELPERS
// ============================================================
export function isValidMessageEvent(payload) {
    const provider = detectProvider(payload);
    if (provider === "waha") {
    ...
    if (provider === "zapi") {
        ...
        if (payload.notification)
            return false; // Ignorar notificações
        if (payload.waitingMessage)
            return false; // Mensagem não decriptada
        const hasContent = !!(payload.text?.message ||
            payload.image ||
            payload.audio ||
            payload.video ||
            payload.document ||
...
export function isFromMe(payload) {
    const provider = detectProvider(payload);
    if (provider === "waha") {
