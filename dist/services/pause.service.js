 ...
 */
export async function pauseChat(conversationId, reason, ttlMinutes = 30, pausedBy = "system") {
    try {
 ...
 */
export async function pauseChatByPhone(chatId, reason, ttlMinutes = 30, pausedBy = "system") {
    try {
        const pausedUntil = new Date(Date.now() + ttlMinutes * 60 * 1000);
        const result = await supabaseService.request("PATCH", "conversations", {
 ...
 */
export async function resumeChat(conversationId) {
    try {
        const result = await supabaseService.request("PATCH", "conversations", {
            query: `id=eq.${conversationId}`,
            body: {
 ...
 */
export async function resumeChatByPhone(chatId) {
    try {
        await supabaseService.request("PATCH", "conversations", {
            query: `chat_id=eq.${encodeURIComponent(chatId)}`,
            body: {
 ...
 */
export async function isChatPaused(conversationId) {
    try {
        const result = await supabaseService.request("GET", "conversations", {
 ...
 */
export async function isChatPausedByPhone(chatId) {
    try {
        // Tenta primeiro pelo chat_id exato
        let result = await supabaseService.request("GET", "conversations", {
            query: `chat_id=eq.${encodeURIComponent(chatId)}&select=paused_until&limit=1`,
        });
        // Se não encontrou, tenta pelo phone
        if (!result || !result[0]) {
            const cleanPhone = chatId.replace(/@(c\.us|g\.us|lid)$/, "");
            result = await supabaseService.request("GET", "conversations", {
 ...
 */
export async function getPauseInfo(conversationId) {
    try {
        const result = await supabaseService.request("GET", "conversations", {
