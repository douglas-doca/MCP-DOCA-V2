...
type ChatPayload = {
  ...
  session_id?: string;
  message?: string;
  channel?: string; // "landing_chat" | "whatsapp" | outros
  ui_mode?: "real" | "instant";
...
function normalizeChannel(v: any): string {
  const ch = safeString(v).trim().toLowerCase();

  if (ch === "landing" || ch === "landing_page" || ch === "landingpage") return "landing_chat";
  if (ch === "landing_chat") return "landing_chat";
  if (ch === "whatsapp") return "whatsapp";

  return ch || "whatsapp";
}

function normalizeUiMode(v: any): "real" | "instant" {
  const u = safeString(v).trim().toLowerCase();
  return u === "instant" ? "instant" : "real";
}

function pickMessage(payload: ChatPayload): string {
 ...
 *  - "landing:<sessionId>"
 */
function buildLandingIdentity(sessionId: string): { phone: string; chatId: string; sessionId: string } {
...
function fallbackNoCreditsReply(): { reply: string; responsePlan: any } {
  const reply =
...
export class WebhookService {
  async handleChat(payload: ChatPayload): Promise<any> {
    ...
    logger.info(
      "API CHAT received",
      { phone, chatId, channel, ui_mode: uiMode, session_id: resolvedSessionId || null },
      "WEBHOOK"
    );
