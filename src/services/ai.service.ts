...
export class AIService {
  ...
  private providerConfigs: Record<AIProvider, ProviderConfig> = {
    openai: {
  ...
  async analyzeIntent(message: string): Promise<{
    ...
    const response = await this.chat(
      message,
      `Você é um analisador de intenções. Analise a mensagem e retorne APENAS um JSON válido com:
{
...
Responda SOMENTE com o JSON, sem texto adicional.`
  ...
  async analyzeSentiment(message: string): Promise<{
    ...
    const response = await this.chat(
      message,
      `Você é um analisador de sentimentos. Analise a mensagem e retorne APENAS um JSON válido com:
{
  "sentiment": "positive" | "neutral" | "negative",
...
- Sempre tente direcionar para uma conversão (agendamento, contato, venda)`;
  ...
  async summarizeConversation(messages: AIMessage[]): Promise<string> {
    ...
    return this.chat(
      conversationText,
      `Resuma esta conversa em 2-3 frases, destacando:
...
3. Próximos passos ou conclusão`
  ...
  async qualifyLead(messages: AIMessage[]): Promise<{
    score: number;
    interests: string[];
    objections: string[];
    nextAction: string;
    ...
    const response = await this.chat(
      conversationText,
      `Analise esta conversa de vendas e retorne APENAS um JSON válido com:
{
...
Responda SOMENTE com o JSON, sem texto adicional.`
  ...
  getConfig(): { provider: string; model: string; maxTokens: number; temperature: number } {
    return {
      provider: this.provider,
      model: this.model,
