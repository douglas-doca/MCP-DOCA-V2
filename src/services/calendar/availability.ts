...
type BuildSlotsParams = {
  ...
  minBufferMinutes?: number; // default 90 (não sugerir agora-agora)
};

export function buildCandidateSlots(params: BuildSlotsParams): TimeSlot[] {
  const {
    timezone,
    days,
    durationMinutes,
...
type Interval = { start: string; end: string }; // ISO strings

export function filterBusySlots(candidateSlots: TimeSlot[], busy: Interval[]): TimeSlot[] {
...
export function formatSuggestionLabel(startISO: string, timezone: string): string {
  ...
  if (dt.startOf("day").equals(today)) prefix = "Hoje";
  else if (dt.startOf("day").equals(tomorrow)) prefix = "Amanhã";
